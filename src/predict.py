...
def load_models():
              ...
              if not paths.IF_PATH.exists():
                                raise FileNotFoundError("Models not found. Run training pipeline first.")

              _models["isolation_forest"] = joblib.load(paths.IF_PATH)

    if paths.RF_PATH.exists():
                      _models["random_forest"] = joblib.load(paths.RF_PATH)
                  if paths.SCALER_PATH.exists():
                                    _scaler = joblib.load(paths.SCALER_PATH)

    if paths.THRESHOLD_PATH.exists():
                      with open(paths.THRESHOLD_PATH) as f:
                                            _thresh = json.load(f)
