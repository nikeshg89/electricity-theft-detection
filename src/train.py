...
FEAT_COLS  = os.path.join(MODEL_DIR, "feature_columns.pkl")
...
def profile_to_features(profile: np.ndarray) -> np.ndarray:
  ...
  def profiles_to_feature_matrix(profiles: np.ndarray) -> np.ndarray:
    ...
    def train_isolation_forest(X: np.ndarray) -> IsolationForest:
          ...
          print(f"[INFO] Isolation Forest saved -> {IF_PATH}")
          return clf


      def train_random_forest(X_train, y_train) -> RandomForestClassifier:
            print("[INFO] Training Random Forest (supervised) ...")
...
def plot_confusion_matrix(name, y_true, y_pred):
      ...
      plt.tight_layout()
      out = os.path.join(SCREENSHOTS, f"cm_{name.lower().replace(' ','_')}.png")
  ...
  def plot_score_distribution(if_model, X, y):
        ...
        ax.set_ylabel("Count")
        ax.legend()
        out = os.path.join(SCREENSHOTS, "anomaly_score_distribution.png")
        plt.tight_layout()
        plt.savefig(out, dpi=150)
        plt.close()
    ...
    def plot_feature_importance(rf_model):
          ...
          plt.tight_layout()
          out = os.path.join(SCREENSHOTS, "feature_importance.png")
          plt.savefig(out, dpi=150)
          plt.close()
      ...
      def run_training():
            ...
            joblib.dump(FEATURE_COLS, FEAT_COLS)
    ...
    print(f"[OK] Metrics exported -> {metrics_path}")
